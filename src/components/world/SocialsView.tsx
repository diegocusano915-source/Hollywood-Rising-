...
import { ArrowLeft, X, Star, Instagram as InstaIcon, Youtube, Facebook, Briefcase, MessageSquare as RedditIcon, Send, Crown, Sparkles } from 'lucide-react';
...
export const SocialsView: React.FC<SocialsViewProps> = ({ onBack }) => {
  ...
  if (selected) {
    ...
    switch (selected) {
      case 'twitter': return <TwitterXView onBack={close} />;
      case 'instagram': return <InstagramView onBack={close} />;
      case 'youtube': return <YouTubeView onBack={close} />;
      case 'facebook': return <FacebookView onBack={close} />;
      case 'marquee': return <MarqueeView onBack={close} />;
      case 'reddit': return <RedditView onBack={close} />;
      case 'telegram': return <TelegramView onBack={close} />;
    }
  }
    ...
    >
      ...
      <div className="p-4 rounded-3xl border border-amber-500/30 bg-gradient-to-br from-amber-950/30 via-black/70 to-black/70 space-y-1">
        ...
        <p className="text-[11px] text-gray-400 leading-relaxed">
          Every platform starts at <strong className="text-white">0 followers</strong> — you build them with real releases,
          posts, awards and scandals. Nothing is simulated.
        </p>
        <div className="flex flex-wrap items-center gap-2 pt-1">
          ...
          <span className="text-[9px] text-gray-500 font-bold">Tap a platform to open it</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2.5">
        {platforms.map((p) => (
          <button
            ...
            className={`p-3 rounded-2xl border bg-gradient-to-br ${p.color} text-left transition-all cursor-pointer hover:scale-[1.03] aspect-square flex flex-col justify-between shadow-lg backdrop-blur-md`}
          >
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-[8px] text-gray-400">{p.tagline}</p>
              <p className="text-[10px] font-black text-emerald-400">{counts[p.id]} <span className="text-[8px] text-gray-500 font-normal">{p.followers}</span></p>
            </div>
          </button>
        ))}
      </div>
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-[9px] text-gray-400">Manage inside any platform — boost your posts, hire a writer, earn ad revenue.</p>
        </div>
