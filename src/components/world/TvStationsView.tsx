/**
 * HOLLYWOOD RISING - TV Stations View (World Ecosystem)
 * TV Interviews checked against player Fame, Movies Released & Lead Roles.
 */
...
export const TvStationsView: React.FC<TvStationsViewProps> = ({ onBack }) => {
  ...
  const [stations, setStations] = useState<TvStation[]>(INITIAL_TV_STATIONS);
  ...
  const handleAcceptInterview = (stationId: string) => {
    ...
    setFeedback(`ACCEPTED TV INTERVIEW ON "${st.name}"! Gained +${st.activeInterviewOffer.fameXpReward} Fame XP & +$${st.activeInterviewOffer.cashReward.toLocaleString()} appearance fee!`);
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-black text-amber-300 bg-amber-500/10 px-3 py-1.5 rounded-xl border border-amber-500/30 flex items-center gap-1.5">
          ...
          Hollywood Television Broadcast Network
        </span>
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-white">TELEVISION NETWORKS & TALK SHOWS</h1>
            <p className="text-xs text-amber-300 font-medium">
              Accept late-night guest appearances, morning show press tours & investigative profiles.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="p-10 rounded-3xl border border-amber-500/30 bg-black/50 text-center space-y-4 shadow-2xl backdrop-blur-md max-w-xl mx-auto my-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400 max-w-md mx-auto">
              Broadcast networks require actors to have fame, completed movies, or lead role credits before scheduling talk show press tours.
            </p>
          </div>
        </div>
      ) : (
            ...
            >
              ...
              <div className="h-36 w-full relative overflow-hidden bg-gray-900">
                ...
                <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-300 font-medium">{st.showName}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 pt-0 space-y-3">
                <p className="text-xs text-gray-400 font-medium">
                  Host: <strong className="text-amber-300">{st.host}</strong>
                </p>
                ...
                {st.activeInterviewOffer ? (
                  <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-extrabold text-amber-300 uppercase text-[10px]">
                        INTERVIEW INVITATION FROM MANAGER
                      </span>
                    ...
                    <div className="flex items-center gap-3 text-xs pt-1">
                      <span className="font-extrabold text-emerald-400">
                        +${st.activeInterviewOffer.cashReward.toLocaleString()} Fee
                      </span>
                      <span className="font-extrabold text-sky-400">
                        +{st.activeInterviewOffer.fameXpReward} Fame XP
                      </span>
                    </div>
                  ...
                  <div className="p-3 rounded-xl bg-black/50 border border-white/5 text-[10px] text-gray-500 text-center font-bold">
                    No active interview offers pending. Check back next week!
                  </div>
                )}
              </div>
            </div>
