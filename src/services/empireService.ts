...
export class EmpireService {
  ...
  public static loadState(player?: Player): EmpireFullState {
    try {
      ...
      if (data) {
        const parsed: EmpireFullState = JSON.parse(data);
        // Ensure array properties exist
        if (!parsed.boardSeats) parsed.boardSeats = [];
        if (!parsed.globalHubs) parsed.globalHubs = [];
        if (!parsed.empireLogs) parsed.empireLogs = [];
        ...
        if (!parsed.acquisitionsCatalog) {
          parsed.acquisitionsCatalog = INITIAL_ACQUISITION_CATALOG;
        }
        
        if (!parsed.legacy) {
          const dummyPlayer = player || ({ id: 'p1', money: 100000, dateWeek: 1, dateYear: 2026 } as Player);
          parsed.legacy = createInitialEmpireState(dummyPlayer).legacy;
        } else {
          if (parsed.legacy.hallOfFameScore === undefined) parsed.legacy.hallOfFameScore = 0;
          if (!parsed.legacy.milestones) parsed.legacy.milestones = [];
          if (parsed.legacy.walkOfFameStar === undefined) parsed.legacy.walkOfFameStar = false;
          if (parsed.legacy.autobiographyPublished === undefined) parsed.legacy.autobiographyPublished = false;
          if (parsed.legacy.lifetimeBoxOffice === undefined) parsed.legacy.lifetimeBoxOffice = 0;
          if (parsed.legacy.businessEmpireValuation === undefined) parsed.legacy.businessEmpireValuation = 0;
          if (parsed.legacy.realEstateValuation === undefined) parsed.legacy.realEstateValuation = 0;
          if (parsed.legacy.philanthropyDonatedTotal === undefined) parsed.legacy.philanthropyDonatedTotal = 0;
          if (parsed.legacy.awardsWonCount === undefined) parsed.legacy.awardsWonCount = 0;
          if (parsed.legacy.totalMoviesActed === undefined) parsed.legacy.totalMoviesActed = 0;
          if (parsed.legacy.totalMoviesDirected === undefined) parsed.legacy.totalMoviesDirected = 0;
          if (parsed.legacy.totalBusinessesCreated === undefined) parsed.legacy.totalBusinessesCreated = 0;
          if (parsed.legacy.totalGlobalHubsBuilt === undefined) parsed.legacy.totalGlobalHubsBuilt = 0;
          if (parsed.legacy.worldRecordsCount === undefined) parsed.legacy.worldRecordsCount = 0;
        }

        if (!parsed.achievements || parsed.achievements.length < 70) {
          const existingMap = new Map((parsed.achievements || []).map((a) => [a.id, a]));
      ...
      console.warn('Failed to parse Empire state from storage', e);
    }
    ...
    const initState = createInitialEmpireState(dummyPlayer);
    EmpireService.saveState(initState);
    return initState;
  }
