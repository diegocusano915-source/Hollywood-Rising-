...
export const NetworkService = {
  ...
  loadState: (player: Player): NetworkFullState => {
    try {
      ...
      if (data) {
        let parsed: NetworkFullState = JSON.parse(data);

        let needsSave = false;

        // ONE-TIME HOTFIX MIGRATION: Purge demo data & override default values
        if (!parsed.migrationVersion || parsed.migrationVersion < 2) {
          // 1. VAULT PURGE: Delete automatically generated placeholder/demo collectibles
          if (parsed.vaultItems && parsed.vaultItems.length > 0) {
            parsed.vaultItems = parsed.vaultItems.filter((item) => {
              const id = item.id || '';
              // Remove placeholder demo items
              if (
          ...
          if (!parsed.bankAccount) {
            parsed.bankAccount = createInitialNetworkState(player).bankAccount;
          } else {
            // Reset credit score to 320 only if it was the unearned legacy default 720 with no transactions
            if (
              parsed.bankAccount.creditScore === 720 &&
              (!parsed.bankAccount.transactionHistory || parsed.bankAccount.transactionHistory.length === 0)
            ) {
              parsed.bankAccount.creditScore = 320;
              parsed.bankAccount.reputationRating = 'CCC';
            }

            // Savings balance reset if default mock savings with no deposit transactions
            if (parsed.bankAccount.savingsBalance > 0) {
              const hasSavingsTx = (parsed.bankAccount.transactionHistory || []).some(
                (tx) => tx.category === 'Savings' || tx.description.toLowerCase().includes('savings')
              );
              if (!hasSavingsTx) {
                parsed.bankAccount.savingsBalance = 0;
              }
            }

            // Sync checking balance with player cash
            parsed.bankAccount.checkingBalance = player.money;

            // Ensure credit cards is initialized to empty
            if (!parsed.bankAccount.creditCards) {
              parsed.bankAccount.creditCards = [];
            }
          }

          parsed.migrationVersion = 2;
          needsSave = true;
        }

        // ALWAYS sanitize and ensure loan arrays & credit history stats exist and persist
        if (parsed.bankAccount) {
          parsed.bankAccount.activeLoans = parsed.bankAccount.activeLoans || [];
          parsed.bankAccount.loanHistory = parsed.bankAccount.loanHistory || [];
          parsed.bankAccount.loansRepaidCount = parsed.bankAccount.loansRepaidCount || 0;
          parsed.bankAccount.onTimePaymentsCount = parsed.bankAccount.onTimePaymentsCount || 0;
          parsed.bankAccount.missedPaymentsCount = parsed.bankAccount.missedPaymentsCount || 0;
          parsed.bankAccount.loanDefaultsCount = parsed.bankAccount.loanDefaultsCount || 0;
          parsed.bankAccount.creditAgeWeeks = parsed.bankAccount.creditAgeWeeks || 0;
          parsed.bankAccount.creditScore = Math.min(850, Math.max(300, parsed.bankAccount.creditScore || 320));
        }

        if (needsSave) {
          NetworkService.saveState(parsed);
        }
    ...
    const initState = createInitialNetworkState(player);
    NetworkService.saveState(initState);
    return initState;
  },
